...
def subarray_sum(nums,target):
    ...
    # running sums(as keys) and their corresponding indices 
    # in the array (as values).
    # 
    # Why start with {0:-1}?
    ...
    # running sum as we iterate through array.
    current_sum=0
    ...
    for i,num in enumerate(nums):
        ...
        # Specially, we check if 'current_sum-target' is already a key
    ...
    # If we've gone through the entire list and didn't find any subarray
    # with a sum equal to the target, we return an empty list.
    return []
